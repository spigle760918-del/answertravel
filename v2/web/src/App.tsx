        ...
        <main className="content">
          ...
          {tab === "actions" && <>
            {data.optimizationActionPlan?<>
              ...
              {data.actionFactIntake?<><section className="panel"><div className="panel-head"><div><p className="eyebrow">品牌个性化事实采集</p><h2>流程保持一致，关注主题跟随每个品牌变化</h2></div><div><StatusPill value="诊断驱动" tone="good"/><StatusPill value={data.actionFactIntake.publicationAuthorized?"已授权发布":"未授权发布"} tone="neutral"/></div></div><p>当前行动：{data.actionFactIntake.actionPackageTitle}。系统从 {data.actionFactIntake.sourceFindingCount} 条证据缺口生成 {data.actionFactIntake.focusCount} 个品牌专属关注主题，没有套用全行业固定问卷。</p></section><section className="decision-grid"><article className="panel"><h2>所有品牌共用的证据外壳</h2><ul>{data.actionFactIntake.coreEnvelopeFields.map(x=><li key={x}>{x}</li>)}</ul></article><article className="panel"><h2>每个主题可直接选择</h2><ul><li>现在提供真实资料</li><li>确认与本品牌不适用</li><li>稍后增补并继续保留缺口</li></ul><p className="empty">未提供真实资料前，不生成事实草案。</p></article></section><section className="decision-grid">{data.actionFactIntake.focusAreas.map(x=><article className="panel" key={x.key}><p className="eyebrow">{x.label}</p><h2>{x.prompt}</h2><p>{x.businessReason}</p><h3>可接受依据</h3><ul>{x.acceptableEvidence.map(y=><li key={y}>{y}</li>)}</ul></article>)}</section><section className="decision-grid"><article className="panel"><h2>完整性门禁</h2><ul>{data.actionFactIntake.completenessRules.map(x=><li key={x}>{x}</li>)}</ul></article><article className="panel"><h2>验收目标</h2><ul>{data.actionFactIntake.acceptanceCriteria.map(x=><li key={x}>{x}</li>)}</ul></article></section></>:null}
