...
- V2 用法：作为客户工作台的前端用户体验、运营信息架构、视觉布局以及内容/发布工作流基准。已核验的界面范式包括统一分组侧栏、简洁顶栏、AI 工作台主入口、数据中心、任务/内容/资产/分发管理、页面级主操作、筛选表格、工作队列和明确空状态。AnswerTravel 在此标准上替换为自身的 GEO 监测、证据、诊断、行动和复测业务。
- 不直接承担：AnswerTravel 的多模型观察事实仓、同口径竞品实验和因果复测。
- 许可证决定：AGPL-3.0-only。用户已于 2026-09-17 明确接受 GEOFlow 前端采用 AGPL 开源方式并按固定版本一比一复刻；前端修改版本必须保留许可证、版权信息和网络交互场景下的对应源码提供机制。AnswerTravel 独立后端与 AGPL 前端的边界、部署和源码交付方式按 `48-geoflow-agpl-frontend-integration-adr.md` 执行。
