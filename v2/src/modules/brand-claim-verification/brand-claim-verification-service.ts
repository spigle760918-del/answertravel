...
export class BrandClaimVerificationService{
  ...
  async verifyTenant(tenantId:string):Promise<{answers:number;created:number;idempotent:number;findings:number}>{const inputs=await this.repository.inputs(tenantId);let created=0,idempotent=0,findings=0;
    for(const input of inputs){const hash=createHash("sha256").update(JSON.stringify({answerId:input.answerId,text:input.answerText,truth:input.brandTruthCardId,version:input.brandTruthVersion,rules:"brand-claim-verification.v2"})).digest("hex");if(await this.repository.byInputHash(tenantId,hash)){idempotent++;continue;}
