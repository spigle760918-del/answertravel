...
export type AcceptanceOverview = {
  ...
  websiteRemediationBlueprint:null|{rulesVersion:"website-remediation-blueprint.v1";status:"draft";factLevel:"F4";batchCount:number;taskCount:number;batches:Array<{key:string;title:string;priority:number;status:string;tasks:Array<{id:string;key:string;title:string;pageKeys:string[];reason:string;status:string;allowedFacts:string[];prohibitedClaims:string[];acceptanceCriteria:string[];rollback:string}>}>;implementationAuthorized:false;publicationAuthorized:false;createdAt:string};
...
export class AcceptanceConsoleRepository {
  ...
  async overview(tenantId: string): Promise<AcceptanceOverview> {
    return withTenantTransaction(this.pool, tenantId, async (client) => {
      ...
      const websiteDiagnosis=await client.query(`select * from website_diagnosis_snapshots order by created_at desc,id desc limit 1`);const websiteDiagnosisRow=websiteDiagnosis.rows[0];
      const websiteRemediation=await client.query(`select * from website_remediation_blueprints order by created_at desc,id desc limit 1`);const websiteRemediationRow=websiteRemediation.rows[0];
      ...
      return {
        ...
        websiteDiagnosis:websiteDiagnosisRow?{rulesVersion:websiteDiagnosisRow.rules_version,status:websiteDiagnosisRow.status,factLevel:websiteDiagnosisRow.fact_level,targetCount:websiteDiagnosisRow.target_count,succeededCount:websiteDiagnosisRow.succeeded_count,blockedCount:websiteDiagnosisRow.blocked_count,failedCount:websiteDiagnosisRow.failed_count,pages:websiteDiagnosisRow.pages,strengths:websiteDiagnosisRow.strengths,gaps:websiteDiagnosisRow.gaps,selfReportedClaims:websiteDiagnosisRow.self_reported_claims,publicationAuthorized:websiteDiagnosisRow.publication_authorized,createdAt:websiteDiagnosisRow.created_at.toISOString()}:null,
        websiteRemediationBlueprint:websiteRemediationRow?{rulesVersion:websiteRemediationRow.rules_version,status:websiteRemediationRow.status,factLevel:websiteRemediationRow.fact_level,batchCount:websiteRemediationRow.batch_count,taskCount:websiteRemediationRow.task_count,batches:websiteRemediationRow.batches,implementationAuthorized:websiteRemediationRow.implementation_authorized,publicationAuthorized:websiteRemediationRow.publication_authorized,createdAt:websiteRemediationRow.created_at.toISOString()}:null,
