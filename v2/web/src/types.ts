export type Overview = {
  ...
  websiteRemediationBlueprint:null|{rulesVersion:"website-remediation-blueprint.v1";status:"draft";factLevel:"F4";batchCount:number;taskCount:number;batches:Array<{key:string;title:string;priority:number;status:"ready_for_technical_implementation"|"waiting_facts";tasks:Array<{id:string;key:string;title:string;pageKeys:string[];reason:string;status:"ready_for_technical_implementation"|"waiting_facts";allowedFacts:string[];prohibitedClaims:string[];acceptanceCriteria:string[];rollback:string}>}>;implementationAuthorized:false;publicationAuthorized:false;createdAt:string};
