...
export function createGeoIntelligenceWorker(redisUrl: string, pool: pg.Pool) { const connection = new Redis(redisUrl, { maxRetriesPerRequest: null });
  const service = new GeoIntelligenceService(new GeoIntelligenceRepository(pool)); const worker = new Worker(GEO_INTELLIGENCE_QUEUE, async (job) => {
    ...
    return service.analyzeAnswer(parsed.data.tenantId, answerId); }, { connection, concurrency: 2 });
