...
export class AcceptanceConsoleRepository {
  ...
  async overview(tenantId: string): Promise<AcceptanceOverview> {
    return withTenantTransaction(this.pool, tenantId, async (client) => {
      ...
      const actionFactIntake=await client.query(`select * from action_fact_intake_snapshots order by created_at desc,id desc limit 1`);const actionFactIntakeRow=actionFactIntake.rows[0];
      ...
      return {
        ...
        actionFactIntake:actionFactIntakeRow?{rulesVersion:actionFactIntakeRow.rules_version,status:actionFactIntakeRow.status,factLevel:actionFactIntakeRow.fact_level,actionPackageKey:actionFactIntakeRow.action_package_key,actionPackageTitle:actionFactIntakeRow.action_package_title,personalizationMode:actionFactIntakeRow.personalization_mode,sourceFindingCount:actionFactIntakeRow.source_finding_count,coreEnvelopeFields:actionFactIntakeRow.core_envelope_fields,focusCount:actionFactIntakeRow.focus_count,focusAreas:actionFactIntakeRow.focus_areas,responseOptions:actionFactIntakeRow.response_options,acceptanceCriteria:actionFactIntakeRow.acceptance_criteria,completenessRules:actionFactIntakeRow.completeness_rules,missingInputs:actionFactIntakeRow.missing_inputs,factDraftAuthorized:actionFactIntakeRow.fact_draft_authorized,publicationAuthorized:actionFactIntakeRow.publication_authorized,createdAt:actionFactIntakeRow.created_at.toISOString()}:null,
